...
from elastic_actuation_arm.calibration.environment.experiment_configuration_handler import align_measurements
...
class SpringStiffnessPerturbationCallback(EvaluationCallback):
    ...
    def from_genome(
            self,
            genome: ManipulatorCalibrationGenome
            ):
...
class NRMSEFitnessCallback(EvaluationCallback):
    ...
    def before_step(
        ...
        measurement_keys = ['timestep', 'shoulder_q', 'elbow_q', 'shoulder_qvel', 'elbow_qvel', 'shoulder_qacc',
                            'elbow_qacc', 'shoulder_torque', 'elbow_torque', 'shoulder_load_torque',
                            'elbow_load_torque']
    ...
    def update_evaluation_result(
        ...
        for simulation_trajectory, experiment_config in zip(self._all_measurements, self._experiment_configurations):
            ...
            real_world_trajectory = align_measurements(
                    values=real_world_trajectory,
            ...
            rmse = mean_squared_error(
                    y_true=real_world_trajectory, y_pred=simulation_trajectory, squared=False, multioutput='raw_values'
                    )
...
class LoadTorqueNRMSEFitnessCallback(EvaluationCallback):
    def __init__(
        ...
        self._current_measurements = []
        self._all_measurements = []
        self._measurement_keys = ['shoulder_q', 'elbow_q', 'shoulder_q_vel', 'elbow_q_vel', 'shoulder_torque',
                                  'elbow_torque']
    ...
    def update_evaluation_result(
        ...
        for simulation_trajectory, experiment_config in zip(
                self._all_measurements, self._environment_config.experiment_configurations
                ):
            real_world_trajectory = experiment_config.measurements
            ...
            shoulder_load_torque = calculate_load_torque(
                    joint_index=1, torque=simulation_trajectory[:, 4], vel=shoulder_q_vel, acc=shoulder_q_acc
                    )
            elbow_load_torque = calculate_load_torque(
                    joint_index=2, torque=simulation_trajectory[:, 5], vel=elbow_q_vel, acc=elbow_q_acc
                    )

            simulation_load_torques = np.vstack([shoulder_load_torque, elbow_load_torque]).T

            simulation_trajectory = np.concatenate(
                    [simulation_trajectory, simulation_acceleration, simulation_load_torques], axis=1
                    )
            ...
            mse = mean_squared_error(
                    y_true=real_world_trajectory, y_pred=simulation_trajectory, squared=True, multioutput='raw_values'
                    )
            nrmse = np.sqrt(mse / np.var(real_world_trajectory, axis=0))
        ...
        # Add average NRMSE per dimension to evaluation_result.info for logging purposes
        evaluation_result.info["logging_nrmse"] = np.average(nrmse_per_trajectory, axis=0)
        evaluation_result.info["all_nrmses_per_trajectory"] = nrmse_per_trajectory
...
class TrajectoryPlotCallback(EvaluationCallback):
    def __init__(
        ...
        self._current_measurements = []
        self._all_measurements = []
        self._measurement_keys = ['shoulder_q', 'elbow_q', 'shoulder_q_vel', 'elbow_q_vel',
                                  'shoulder_torque',
                                  'elbow_torque']
...
class TrajectorySaverCallback(EvaluationCallback):
    def __init__(
            self,
            config: EAConfig
            ):
        ...
        self._current_measurements = []
        self._all_measurements = []
        self._measurement_keys = ['timestep', 'shoulder_q', 'elbow_q', 'shoulder_q_vel', 'elbow_q_vel',
    ...
    def update_evaluation_result(
        ...
        shoulder_indices = [i for i, key in enumerate(self._measurement_keys) if "shoulder" in key or key == "timestep"]
        elbow_indices = [i for i, key in enumerate(self._measurement_keys) if "elbow" in key or key == "timestep"]
        ...
        def save_joint_df(
            ...
            columns = [self._measurement_keys[i] for i in indices]
            joint_measurements = measurements[:, indices]

            df = pd.DataFrame(data=joint_measurements, columns=columns)
            df.to_csv(path_or_buf=str(self._base_path / f"{name}.csv"), sep=" ", index=False, header=True)
        ...
        for simulation_trajectory, experiment_config in zip(
            ...
            save_joint_df(
                    name=f"joint_1_trajectory_{experiment_config.identifier}",
                    indices=shoulder_indices,
                    measurements=simulation_trajectory
                    )
            save_joint_df(
                    name=f"joint_2_trajectory_{experiment_config.identifier}",
                    indices=elbow_indices,
                    measurements=simulation_trajectory
                    )
